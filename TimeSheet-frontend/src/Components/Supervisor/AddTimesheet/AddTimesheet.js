 ...
 useEffect(() => {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        // Fetch supervisor's working hours from the backend
        const response = await axios.get(`${serverUrl}/sup/api/working-hours/${supervisorId}`, {
          params: { selectedMonth }
        });

        const savedData = response.data || [];

        // Filter data to match the selected month and year
        ...
        let updatedSubmittedHalf = { firstHalf: false, secondHalf: false }; // ✅ Track submitted halves
        ...
        filteredData.forEach((entry) => {
          ...
          if (day >= 1 && day <= 15) {
            updatedSubmittedHalf.firstHalf = true; // ✅ First half marked as submitted
          } else {
            updatedSubmittedHalf.secondHalf = true; // ✅ Second half marked as submitted
          }
        ...
        setSubmittedHalf(updatedSubmittedHalf); // ✅ Store submitted halves
  ...
  const handleSaveTimesheetData = () => {
    if (validateTimesheetData()) {
      ...
      localStorage.setItem(supervisorId, JSON.stringify([...existingData, formattedData]));
  ...
  const submitTimesheetData = async () => {
    ...
    // ✅ Ensure `formattedData` is properly initialized
    let formattedData = [];
    ...
    if (formattedData.length > 0) {
      try {
        const response = await axios.post(`${serverUrl}/sup/api/working-hours`, formattedData);
        ...
        console.error("Error saving supervisor timesheet data:", error);
        // setTimesheetError("Failed to submit supervisor timesheet. Please try again.");
      }
    } else {
      console.warn("No supervisor data to save.");
    }
  };
