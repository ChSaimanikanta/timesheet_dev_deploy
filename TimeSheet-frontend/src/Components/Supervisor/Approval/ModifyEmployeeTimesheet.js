...
function ModifyEmployeeTimesheet() {
  ...
  const [rejectReason, setRejectReason] = useState(
    "Please reach out superadmin  regarding your timesheet."
  );

  const editTimesheetSuccessModalValue = useSelector(
    (state) => state.modal.value.editTimesheetSuccessModalValue
  );
  const editTimesheetRejectModalValue = useSelector(
    (state) => state.modal.value.editTimesheetRejectModalValue
  );
      ...
      await Promise.all(
        editableData.map(async (data) => {
          let response = await axios.put(
            `${serverUrl}/workinghours/${data.id}`,
            data
          );
        })
      );
    ...
    try {
      ...
      await Promise.all(
        keysArray.map(async (pId) => {
          ...
          return axios.put(
            `${serverUrl}/workinghours/employee/${id}/range/approval?startDate=${startDate}&endDate=${endDate}&status=APPROVED&rejectionReason=${rejectionReason}&projectId=${pId}`
          );
        })
      );
  ...
  async function editTimesheetRejectSave() {
    ...
    try {
      ...
      keysArray.map(async (projectId) => {
        await axios.put(
          `${serverUrl}/workinghours/employee/${id}/range/approval?startDate=${startDate}&endDate=${endDate}&status=REJECTED&rejectionReason=${rejectReason}&projectId=${projectId}`
        );
      });
      ...
      {timesheetData && (
        <div className="AddTimesheet background-clr">
          <div className="AddTimesheet employeeEdit-container pt-4">
            <div className="mt-n3">
              <p className="AddTimesheet fs-4 text-black text-center">Edit Timesheet</p>
            </div>
            ...
            <div className="d-flex justify-content-center">
              <button
                type="button"
                onClick={editTimesheetApproveConfirmation}
              ...
              <button
                type="button"
                onClick={editTimesheetRejectConfirmation}
          ...
          <div>
            ...
            <Modal show={editApproveConfirmationModal}>
              ...
              <Modal.Footer>
                <Button
                  variant="secondary"
                  onClick={editTimesheetApproveCancel}
                >
                  Cancel
                </Button>
